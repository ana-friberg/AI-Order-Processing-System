...
```
┌─────────────────┐    ┌──────────────────┐    ┌─────────────────┐
│   PDF Upload    │───▶│   Claude AI      │───▶│   Priority ERP  │
│   (Flask API)   │    │   Processing     │    │   Integration   │
└─────────────────┘    └──────────────────┘    └─────────────────┘
         │                       │                        │
         ▼                       ▼                        ▼
┌─────────────────┐    ┌──────────────────┐    ┌─────────────────┐
│   File Cache    │    │   Data Extraction│    │   Order Updates │
│   Management    │    │   & Validation   │    │   & Status      │
└─────────────────┘    └──────────────────┘    └─────────────────┘
                                │
                                ▼
                     ┌──────────────────┐
...
### Process Order PDF
**POST** `/process`
- **Purpose**: Process Agilent order confirmation PDFs
...
#### Status Updates
- **"אישור ספק" (Supplier Approval)**: All validations pass perfectly
- **"נשלח לספק" (Sent to Supplier)**: Discrepancies found but processable
...
```
1. PDF Upload → Extract Customer PO from first page
2. Customer PO → Validate existence in Priority ERP system  
3. Priority Query → Get required PARTNAMEs for targeted extraction
4. Full PDF Processing → Claude AI extraction using specific PARTNAMEs
5. Data Validation → Comprehensive validation against Priority data
6. Business Rules → Apply delivery date calculation rules
7. Priority Updates → Update delivery dates and order status
8. Data Storage → Save complete results to MongoDB
9. Response → Return comprehensive processing results
```
...
This project is proprietary software for Agilent order processing automation.
